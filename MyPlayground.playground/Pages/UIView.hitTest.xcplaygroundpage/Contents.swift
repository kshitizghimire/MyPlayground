...
extension UIView {
    ...
    func point2(inside point: CGPoint, with _: UIEvent?) -> Bool {
        point.x >= bounds.origin.x && point.x <= bounds.size.width
            && point.y > bounds.origin.y && point.y <= bounds.size.height
    }
}
